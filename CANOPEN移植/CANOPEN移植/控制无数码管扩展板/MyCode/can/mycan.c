...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
	...
	can_msg.cob_id = RxMessage.StdId;
	can_msg.rtr = RxMessage.RTR;
	can_msg.len = RxMessage.DLC;
	for(i=0; i<can_msg.len; i++)
    {
        can_msg.data[i] = RxMessage.Data[i];
    }
	canDispatch(&gk_Data, &can_msg);    //´¦ÀíÏûÏ¢
}
#endif
...
{		
	...
	CanTxMsg TxMessage;
	TxMessage.StdId = msg->cob_id;			// ±ê×¼±êÊ¶·û 
	TxMessage.ExtId = 0;			        // ÉèÖÃÀ©Õ¹±êÊ¾·û 
	TxMessage.IDE = CAN_Id_Standard; 	    // ±ê×¼Ö¡
	TxMessage.RTR = msg->rtr;		        // Êı¾İÖ¡
	TxMessage.DLC = msg->len;				// Òª·¢ËÍµÄÊı¾İ³¤¶È
	for(i=0; i<msg->len; i++)
    {
